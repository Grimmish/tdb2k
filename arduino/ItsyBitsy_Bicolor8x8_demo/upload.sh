...
#FIXME:
#FIXME: I started writing this to work around a papercut in the arduino CLI
#FIXME: where it wouldn't wait long enough between putting the board in
#FIXME: programming mode and shoving code at it. I eventually realized that
#FIXME: I was rewriting a complete ./arduino tool replacement from scratch
#FIXME: just to fix one bug, and it turns out that ./arduino does an awful
#FIXME: log of per-board customization in its workflow that I am not
#FIXME: interested in recreating. So this goes on the shelf.

###########################################
# Config (see below for more explanation)

PROJECTNAME=ItsyBitsy_Bicolor8x8_demo
DEVPATH=/dev/ttyACM0
